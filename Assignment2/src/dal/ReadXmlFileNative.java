...
public class ReadXmlFileNative {
	
	private GasStationObject gasStation;
	
	public ReadXmlFileNative(String filePath) {
		
		try {
			
			gasStation = new GasStationObject(0);
    ...
    public GasStationObject getGasStation()
    {
    	return gasStation;
    }
    ...
    private void readPumpData(Document doc){
    	...
    	for(int i=0;i<numOfPumps;i++){
    		gasStation.addPump(new PumpObject(i, pricePerLiter));
    	}
    }
    ...
    private void readMainFuelPoolData(Document doc) {
    	...
    	gasStation.setFuelPoolObject(new FuelPoolObject(0, maxCapacity, currentCapacity));
    }
    ...
    private void readCleaningServiceData(Document doc) {
    	...
    	gasStation.setCleaningServiceObject(new CleaningServiceObject(0, numOfTeams, price, secondsPerAutoClean));
    }
    ...
    private void readCarsData(Document doc) {
    	...
    	for(int i=0;i<nList.getLength();i++)
    	{
    		...
    		if(wantCleaning && pumpNum == 0) {
    			...
    			gasStation.addCar(new CarObject(id, true, false, 0));
        	}
    		else if (!wantCleaning && pumpNum !=0) {
    			
    			gasStation.addCar(new CarObject(id, false, true, 0, pumpNum));
    		}
    		else {
    			gasStation.addCar(new CarObject(id, true, true, 0, pumpNum));
    		}
