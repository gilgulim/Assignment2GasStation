...
public class GasStationMySqlConnection implements IDataBaseConnection {
	...
	public boolean insertGasStationHistoryRecord(
		...
		try {
			...
			if (historyRecord.getActionType() == ActionType.Enter
					|| historyRecord.getActionType() == ActionType.Exit) {
				insertQuery = String.format(
						"INSERT INTO `gasstationhistorylog` "
								+ "(CreationTime, CarID, ActionTypeID) "
								+ "VALUE ('%s', %s, %s)", dateTime,
						historyRecord.getCarId(), historyRecord.getActionType()
								.ordinal());
			} else {
				insertQuery = String
						.format("INSERT INTO `gasstationhistorylog` "
								+ "(CreationTime, CarID, ActionTypeID, ServiceEntityTypeID, ServiceEntityID) "
								+ "VALUE ('%s', %s, %s, %s, %s)", dateTime,
								historyRecord.getCarId(), historyRecord
										.getActionType().ordinal(),
	...
	public boolean insertCar(CarObject car) {
		...
		try {
			...
			if (car.getWantFuel()) {
				insertQuery = String
						.format("INSERT INTO `cars` (CarID, IsCarReqFuel, IsCarReqWash, CarFuelQuantity, CarPumpID) VALUES (%s, %s, %s, %s, %s)",
								car.getId(), car.getWantFuel(),
								car.getWantCleaning(), car.getNumOfLiters(),
								car.getPumpNum());
			} else {
				insertQuery = String
						.format("INSERT INTO `cars` (CarID, IsCarReqFuel, IsCarReqWash) VALUES (%s, %s, %s)",
								car.getId(), car.getWantFuel(),
								car.getWantCleaning());
			}
	...
	public boolean insertGasStation(GasStationObject gasStation) {
		...
		try {
			...
			int insideTeams = gasStation.getCleaningServiceObject()
					.getNumOfInsideTeams();
			int price = (int) gasStation.getCleaningServiceObject().getPrice();
			insertQuery = String
					.format("INSERT INTO cleanservices (CleanServiceID ,NumOfInsideTeams, CleanServicePrice) VALUES (1, %s, %s)",
							insideTeams, price);
			statement.executeUpdate(insertQuery);
			...
			insertQuery = String
					.format("INSERT INTO mainfuelpool (MainFuelPoolID, MaxCapacity) VALUES (1, %s)",
							maxCapacity);
			statement.executeUpdate(insertQuery);
			...
			for (PumpObject pump : gasStation.getPumpsList()) {
				...
				insertQuery = String
						.format("INSERT INTO `pumps` (PumpID, PumpPricePerLiter) VALUES (%s, %s)",
								pump.getId(), pump.getPricePerLiter());
				statement.executeUpdate(insertQuery);

				insertQuery = String
						.format("INSERT INTO `gasstationpumprelationship` (GasStationID, PumpID) VALUES (%s, %s)",
								1, pump.getId());
				statement.executeUpdate(insertQuery);
			}
	...
	private void insertCarHistoryLog(CarObject car) {
		insertGasStationHistoryRecord(new GasStationHistoryRecord(car.getId(),
				ActionType.Enter, null, null));
	}
	...
	public GasStationObject getGasStationById(int gasStationId) {
		...
		try {
			...
			ResultSet rs = statement.executeQuery(String.format(
					"SELECT * FROM gasstation WHERE GasStationID = %s",
					gasStationId));

			while (rs.next()) {
				gasStation = new GasStationObject(gasStationId);

				// TODO: Read all the other parts of the GasStation object from
				// the DB
			}
	...
	public int getLittersByCarId(int carId) {
		...
		try {
			Statement statement = connection.createStatement();
			ResultSet rs = statement.executeQuery(String.format(
					"SELECT * FROM cars WHERE CarID = %d", carId));
	...
	public int getPricePerLittersByPumpId(int pumpId) {
		...
		try {
			Statement statement = connection.createStatement();
			ResultSet rs = statement.executeQuery(String.format(
					"SELECT * FROM pumps WHERE PumpID = %d", pumpId));
	...
	public ArrayList<GasStationHistoryRecord> getStatistics(
		...
		try {
			...
			while (rs.next()) {
				records.add(new GasStationHistoryRecord(rs.getInt("CarID"),
						ActionType.values()[rs.getInt("ActionTypeID")],
						ServiceEntityType.values()[rs
								.getInt("ServiceEntityTypeID")], rs
								.getInt("ServiceEntityID"), rs
